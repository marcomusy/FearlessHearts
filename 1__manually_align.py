...
import heart_database, os

#########################################################################
dset = heart_database.dataset_ko
group = 'set2829' # do first timecourse then all the others
threshold = 60
...
for i, s in enumerate(volfiles):
    ...
    printc("(Over)write "+matfile+' [y/n/break/abort]? ', c=i+2, end=''); r=input()
    if 'y' in r:
